...
(function () {
    ...
    var getContent = function (code) {
        ...
        switch (code) {
            ...
            case '20':
                ...
                '</tr>' +
                '<tr>' +
                '<td colspan="2"><label for="page20_imageX">图片X偏移：</label><input type="text" id="page20_imageX" value="0" /></td><td colspan="2"><label for="page20_lat">经度：</label><input type="text" id="page20_lat" value="0"/></td>' +
                '</tr>' +
                '<tr>' +
                '<td colspan="2"><label for="page20_imageY">图片Y偏移：</label><input type="text" id="page20_imageY" value="0"/></td><td colspan="2"><label for="page20_lng">纬度：</label><input type="text" id="page20_lng" value="0"/></td>' +
                '</tr>' +
                '<tr>' +
            ...
            case '21':
                ...
                '</tr>' +
                '<tr>' +
                '<td><label for="page21_zoomlevels">显示级别：</label></td><td colspan="2"><input type="text" id="page21_zoomlevels" value="[1,18]" /><em>格式如输入框示例，在*到*级显示船舶</em></td>' +
                '</tr>' +
                '</table>' +
                '</fieldset>' +
                '<fieldset><legend>顶点</legend>' +
                '<table><tr>' +
                '<td><label for="page21_lineLatLng">坐标：</label></td><td colspan="2"><input type="text" id="page21_lineLatLng" value="内部指定" /></td>' +
                '</tr>' +
                '</table>' +
                '</fieldset>' +
                ...
                '</tr>' +
                '<tr>' +
                '<td><label for="page21_labelZoomlevels">标签显示级别：</label></td><td><input type="text" id="page21_labelZoomlevels" value="[1,18]"/></td>' +
                '</tr>' +
                '<tr>' +
            ...
            case '22':
                ...
                '<fieldset><legend>顶点</legend>' +
                '<table><tr>' +
                '<td><label for="page22_polygonLatLng">坐标：</label></td><td colspan="2"><input type="text" id="page22_polygonLatLng" value="内部指定" /></td>' +
                '</tr>' +
                '</table>' +
                '</fieldset>' +
                '<fieldset><legend>设置边线样式</legend>' +
            ...
            case '25':
                title += '开启与关闭气象';
                content += '<p>只有key拥有气象服务权限，才能使用气象服务。</p>' +
                    '<p>当有气象服务权限时，会在地图右上角，出现一个气象服务的开启与关闭按钮，点击即可开启气象，再一次点击则关闭气象。无需写任何脚本代码。</p>' +
                    '<p>若要使用气象服务，请联系船讯网。</p>';
                break;
            case '27':
                title += '开启与关闭潮汐';
                content += '<p>只有key拥有潮汐服务权限，才能使用潮汐服务。</p>' +
                    '<p>当有潮汐服务权限时，会在地图右上角，出现一个潮汐服务的开启与关闭按钮，点击即可开启潮汐，再一次点击则关闭潮汐。</p>' +
                    '<p>若要使用潮汐服务，请联系船讯网。<a href="/APIHTML/tide.htm">查看全部观测点</a></p>';
                break;
            case '28':
                title += '开启与关闭台风';
                content += '<p>只有key拥有台风服务权限，才能使用台风服务。</p>' +
                    '<p>当有台风服务权限时，会在地图右上角，出现一个台风服务的开启与关闭按钮，点击即可开启台风，再一次点击则关闭台风。</p>' +
                    '<p>若要使用台风服务，请联系船讯网。</p>';
                break;
            case '26':
                title += '如何使用XWin弹出窗组件';
                content += '<p>详细脚本代码请点击“代码”选项卡查看。</p>' +
                    '<div class="btnLink"><a href="javascript:void(0)" onclick="action.openWin();return false;">弹 出</a><a href="javascript:void(0)" onclick="action.closeWin();return false;">关 闭</a></div>';
                break;
            case '29':
                title += '获取指定船舶靠港记录';
                //初始化日期与时间选择器
                var nDate = new Date();
                var sDateTSpan = nDate.setDate(nDate.getDate() - 30); //30天前
                var startDate = new Date(sDateTSpan);
                content += '<p id="page29_p1"><label for="page29_shipId" style="width:55px;">船舶 ID：</label><input type="text" id="page29_shipId" placeholder="选填" value="412473010"/>' +
                '<label for="page29_imo" style="width:76px;">船舶 IMO：</label><input type="text" id="page29_imo" placeholder="选填" value=""/>' +
                '<label for="page29_name" style="width:70px;">船名：</label><input type="text" id="page29_name" placeholder="选填" value=""/></p>' +
                '<p id="page29_p2"><label for="page29_callsign" style="width:55px;">呼号：</label><input type="text" id="page29_callsign" placeholder="选填" value=""/>' +
                '<label style="width:76px;" for="page29_startTime">起始时间：</label><input type="text" id="page29_startTime" value="' + view.formatDateToString(startDate).substr(0, 10) + ' 00:00:00' + '"/>' +
                '<label style="width:70px;" for="page29_endTime">结束时间：</label><input type="text" id="page29_endTime" value="' + view.formatDateToString(new Date()) + '"/></p>' +
                '<div class="btnLink"><a href="javascript:void(0)" style="margin: 5px 5px;" onclick="action.searchCall();return false;">获 取</a></div>' +
                '<div class="trackMsg"><span id="callMsg"></span></div>' +
                '<div id="page29_result" ></div>';
                break;
            case '30':
                title += '获取指定港口靠港记录';
                //初始化日期与时间选择器
                var nDate = new Date();
                var sDateTSpan = nDate.setDate(nDate.getDate() - 7); //七天前
                var startDate = new Date(sDateTSpan);
                content += '<p id="page30_p1"><label for="page30_portId" style="width:99px;">港口 ID：</label><input type="text" id="page30_portId" placeholder="选填" value="13433"/>' +
                '<label for="page30_portCode" style="width:99px;">港口 Code：</label><input type="text" id="page30_portCode" placeholder="选填" value="CNQIN"/></p>' +
                '<p id="page30_p2"><label style="width:99px;" for="page30_startTime">起始时间：</label><input type="text" id="page30_startTime" value="' + view.formatDateToString(startDate).substr(0, 10) + ' 00:00:00' + '"/>' +
                '<label style="width:99px;" for="page30_endTime">结束时间：</label><input type="text" id="page30_endTime" value="' + view.formatDateToString(new Date()) + '"/></p>' +
                '<div class="btnLink" ><a href="javascript:void(0)" style="margin: 5px 5px;" onclick="action.searchPortCall();return false;">获 取</a></div>' +
                '<div class="trackMsg"><span id="callMsg"></span></div>' +
                '<div id="page30_result" ></div>';
                break;
            case '31':
                title += '获取指定船舶挂靠指定港口靠港记录';
                //初始化日期与时间选择器
                var nDate = new Date();
                var sDateTSpan = nDate.setDate(nDate.getDate() - 30); //七天前
                var startDate = new Date(sDateTSpan);
                content += '<p id="page31_p1"><label for="page31_shipId" >船舶 ID：</label><input type="text" id="page31_shipId" placeholder="选填" value="412473010"/>' +
                '<label for="page31_imo" style="width:77px">船舶 IMO：</label><input type="text" id="page31_imo" placeholder="选填" value=""/>' +
                '<label for="page31_name" >船名：</label><input type="text" id="page31_name" placeholder="选填" value=""/>' +
                '<label for="page31_callsign" >呼号：</label><input type="text" id="page31_callsign" placeholder="选填" value=""/></p>' +
                '<p id="page31_p2"><label style="width:60px;" for="page31_portId">港口 ID：</label><input type="text" id="page31_portId" value="210991,27464" placeholder="逗号分隔的港口id"/></p>' +
                '<p id="page31_p3"><label style="width:60px;" for="page31_startTime">起始时间：</label><input type="text" id="page31_startTime" value="' + view.formatDateToString(startDate).substr(0, 10) + ' 00:00:00' + '"/>' +
                '<label style="width:60px;margin-left:10px" for="page31_endTime">结束时间：</label><input type="text" id="page31_endTime" value="' + view.formatDateToString(new Date()) + '"/>' +
                '<label style="width:60px;margin-left:10px" for="page31_datetype">时间格式：</label><select id="page31_datetype"><option value="1">当地时区</option><option value="2">北京时区</option><option value="3">零时区</option></select></p>' +
                '<div class="btnLink"><a href="javascript:void(0)" style="margin: 5px 5px;" onclick="action.searchShipPtcall();return false;">获 取</a></div>' +
                '<div class="trackMsg"><span id="callMsg"></span></div>' +
                '<div id="page31_result" ></div>';
                break;
            case '32':
                title += '获取船舶挂靠当前状态';
                content += '<p id="page32_p1"><label for="page32_shipId" >船舶 ID：</label><input type="text" id="page32_shipId" placeholder="选填" value="352109000"/>' +
                '<label for="page32_imo" style="width:77px">船舶 IMO：</label><input type="text" id="page32_imo" placeholder="选填" value=""/></p>' +
                '<p id="page32_p2"><label for="page32_name" >船名：</label><input type="text" id="page32_name" placeholder="选填" value=""/>' +
                '<label for="page32_callsign" style="width:77px">呼号：</label><input type="text" id="page32_callsign" placeholder="选填" value=""/></p>' +
                '<div class="btnLink"><a href="javascript:void(0)" style="margin: 5px 5px;" onclick="action.searchShipcallInfo();return false;">获 取</a></div>' +
                '<div class="trackMsg"><span id="callMsg"></span></div>' +
                '<div id="page32_result" ></div>';
                break;
            case '33':
                title += '获取船舶档案';
                content += '<p id="page33_p1">' +
                '<input type ="hidden" id="page33_shipId" value=""/>' +
                '<input type ="hidden" id="page33_name" value=""/>' +
                '<input type ="hidden" id="page33_callsign" value=""/>' +
                '<label for="page33_imo" >船舶 IMO：</label>' +
                '<select id="page33_imo"><option value="9575498">9575498</option><option value="9746035">9746035</option><option value="8501672">8501672</option><option value="9611694">9611694</option></select>' +
                '<div class="btnLink"><a href="javascript:void(0)" style="margin: 5px 5px;" onclick="action.searchShipRecord();return false;">获 取</a></div>' +
                '<div class="trackMsg"><span id="callMsg"></span></div>' +
                '<div id="page33_result" ></div>';
                break;
        }
        content += '</div>';
    ...
    action = {
        ...
        showMap: function () {
            ...
            mapOptions.hideBar = !!window.hideBar; //新增隐藏海洋气象隐藏参数
            mapOptions.hidePort = !!window.hidePort; //新增隐藏港口参数
        ...
        searchTrack: function () {
            ...
            if (srtdate >= enddate) {
                view.setTrackMsg('开始时间必须小于结束时间');
                return;
            }
            if (srtdate + 32 * 24 * 3600 < new Date().getTime() / 1000) {
                view.setTrackMsg('只能查询最近一个月的轨迹');
                return;
            }
        ...
        //查询船舶挂靠港历史
        searchCall: function () {
            var srttime = view.G('page29_startTime').value, endtime = view.G('page29_endTime').value;
            //起始日期对象
            var srtdate = new Date(parseInt(srttime.substr(0, 4), 10),
                parseInt(srttime.substr(5, 2), 10) - 1,
                parseInt(srttime.substr(8, 2), 10),
            ...
            enddate = enddate.getTime() / 1000; //结束时间，秒数
            if (srtdate >= enddate) {
                view.setCallMsg('开始时间必须小于结束时间');
                return;
            }
            var resultDoc = view.G('page29_result');
            if ((enddate - srtdate) / (24 * 3600) > 31) {
                view.setCallMsg('最大允许查询30天内的挂靠，请重新指定时间区间！');
                resultDoc.innerHTML = "";
                return;
            }
            var calls = new shipxyAPI.PortOfCallByShip();
            view.setCallMsg('正在查询挂靠港历史，请稍候...');
            var c1 = view.G('page29_shipId').value;
            var c2 = view.G('page29_imo').value;
            var c3 = view.G('page29_name').value;
            var c4 = view.G('page29_callsign').value;
            var c5 = srtdate;
            var c6 = enddate;
            //获取挂靠港历史数据   shipId, imo,shipname,startTime, endTime

            calls.getCalls({ shipId: c1, imo: c2, shipname: c3, callsign: c4, startTime: c5, endTime: c6 }, function (status) {
                if (status == 0) {//成功
                    view.setCallMsg('');
                    var data = this.data;
                    if (data && data.records && data.records.length > 0) {//有数据
                        data.records.reverse();
                        var c = '', len = 0, ndata = data.records;
                        c += '<table id="page31_table" class="dataTable" cellspacing="0"><caption style="">查询结果<span>(' + (ndata.length ? ndata.length : 0) + ')</span></caption>' +
                        '<thead><tr><th scope="col">港口中文</th><th scope="col">港口英文</th><th scope="col">港口ID</th><th scope="col">港口CODE</th><th scope="col">码头名称</th><th scope="col">国家中文</th>' +
                        '<th scope="col">国家英文</th><th scope="col">国家CODE</th><th scope="col">ATA</th><th scope="col">ATB</th><th scope="col">ATD</th>' +
                        '<th scope="col">到港吃水</th><th scope="col">离港吃水</th><th scope="col">港口作业时间</th><th scope="col">码头作业时间</th>' +
                        '</tr></thead><tbody>';
                        var d = null, i = 0;
                        for (len = ndata.length; i < len; i++) {
                            d = ndata[i];
                            c += '<tr class="' + (i % 2 == 1 ? 'odd' : '') + '">' +
                            '<td title="' + d.portname_cn + '">' + d.portname_cn + '</td><td title="' + d.portname_en + '">' + d.portname_en + '</td><td title="' + d.portid + '">' + d.portid + '</td><td title="' + d.portcode + '">' + d.portcode + '</td><td title="' + d.terminalname + '">' + d.terminalname + '</td><td title="' + d.country_cn + '">' + d.country_cn + '</td>' +
                            '<td title="' + d.country_en + '">' + d.country_en + '</td><td title="' + d.countrycode + '">' + d.countrycode + '</td><td title="' + d.ata + '">' + d.ata + '</td><td title="' + d.atb + '">' + d.atb + '</td><td title="' + d.atd + '">' + d.atd + '</td>' +
                            '<td title="' + d.arrivaldraught + '">' + d.arrivaldraught + '</td><td title="' + d.departuredraught + '">' + d.departuredraught + '</td><td title="' + d.stayinport + '">' + d.stayinport + '</td><td title="' + d.stayinterminal + '">' + d.stayinterminal + '</td>' +
                            '</tr>';
                        }

                        c += '</tbody></table>';
                        resultDoc.innerHTML = c; //生成表格
                    } else {//无数据
                        view.setCallMsg('暂无挂靠港历史数据');
                        resultDoc.innerHTML = "";
                    }
                } else {//失败
                    view.setCallMsg('查询挂靠港历史数据出错');
                    resultDoc.innerHTML = "";
                }
            });
        },
        //查询指定港口挂靠港历史
        searchPortCall: function () {
            var srttime = view.G('page30_startTime').value, endtime = view.G('page30_endTime').value;
            //起始日期对象
            var srtdate = new Date(parseInt(srttime.substr(0, 4), 10),
                parseInt(srttime.substr(5, 2), 10) - 1,
                parseInt(srttime.substr(8, 2), 10),
            ...
            enddate = enddate.getTime() / 1000; //结束时间，秒数
            if (srtdate >= enddate) {
                view.setCallMsg('开始时间必须小于等于结束时间相同');
                return;
            }
            var resultDoc = view.G('page30_result');
            if ((enddate - srtdate) / (24 * 3600) > 8) {
                view.setCallMsg('请重新指定时间区间！');
                resultDoc.innerHTML = "";
                return;
            }
            var calls = new shipxyAPI.PortOfCallByPort();
            view.setCallMsg('正在查询挂靠港历史，请稍候...');
            var c1 = view.G('page30_portId').value;
            var c2 = view.G('page30_portCode').value;
            var c3 = srtdate;
            var c4 = enddate;
            //获取挂靠港历史数据   portId, portCode,startTime, endTime
            calls.getCalls({ portId: c1, portCode: c2, startTime: c3, endTime: c4 }, function (status) {
                if (status == 0) {//成功
                    view.setCallMsg('');
                    var data = this.data;
                    if (data && data.records && data.records.length > 0) {//有数据
                        data.records.reverse();
                        var c = '', len = 0, ndata = data.records.slice(0, 20);//取最新的前20个数据
                        c += '<table id="page30_table" class="dataTable" cellspacing="0"><caption style="">查询结果<span>(' + (ndata.length ? ndata.length : 0) + ')</span></caption>' +
                        '<thead><tr><th scope="col">船名</th><th scope="col">呼号</th><th scope="col">IMO</th><th scope="col">MMSI</th><th scope="col">船类中文</th>' +
                        '<th scope="col">船类英文</th><th scope="col">码头名称</th>' +
                        '<th scope="col">ATA</th><th scope="col">ATB</th><th scope="col">ATD</th>' +
                        '<th scope="col">到港吃水</th><th scope="col">离港吃水</th><th scope="col">港口作业时间</th><th scope="col">码头作业时间</th>' +
                        '</tr></thead><tbody>';
                        var d = null, i = 0;
                        for (len = ndata.length; i < len; i++) {
                            ...
                            var e = d.currentport || {};
                            c += '<tr class="' + (i % 2 == 1 ? 'odd' : '') + '">' +
                            '<td title="' + d.shipname + '">' + d.shipname + '</td><td title="' + d.callsign + '">' + d.callsign + '</td><td title="' + d.imo + '">' + d.imo + '</td><td title="' + d.mmsi + '">' + d.mmsi + '</td><td title="' + d.shiptype_cn + '">' + d.shiptype_cn + '</td>' +
                            '<td title="' + d.shiptype_en + '">' + d.shiptype_en + '</td><td title="' + e.terminalname + '">' + e.terminalname + '</td>' +
                            '<td title="' + e.ata + '">' + e.ata + '</td><td title="' + e.atb + '">' + e.atb + '</td><td title="' + e.atd + '">' + e.atd + '</td>' +
                            '<td title="' + e.arrivaldraught + '">' + e.arrivaldraught + '</td><td title="' + e.departuredraught + '">' + e.departuredraught + '</td><td title="' + e.stayinport + '">' + e.stayinport + '</td><td title="' + e.stayinterminal + '">' + e.stayinterminal + '</td>' +
                            '</tr>';
                        }
        ...
        //查询船舶挂靠指定港历史
        searchShipPtcall: function () {
            var srttime = view.G('page31_startTime').value, endtime = view.G('page31_endTime').value, datetype = view.G('page31_datetype').value;
            //起始日期对象
            var srtdate = new Date(parseInt(srttime.substr(0, 4), 10),
                parseInt(srttime.substr(5, 2), 10) - 1,
                parseInt(srttime.substr(8, 2), 10),
            ...
            var sUTC = srtdate.toUTCString();

            srtdate = srtdate.getTime() / 1000; //起始时间，秒数
            enddate = enddate.getTime() / 1000; //结束时间，秒数

            if (datetype != 1) {
                var offsethour = (datetype == 2 ? 8 : 0);//北京时间 +8
                //起始日期对象
                srtdate = Date.UTC(parseInt(srttime.substr(0, 4), 10),
                    parseInt(srttime.substr(5, 2), 10) - 1,
                    parseInt(srttime.substr(8, 2), 10),
                    parseInt(srttime.substr(11, 2), 10) + offsethour,
                    parseInt(srttime.substr(14, 2), 10), 00) / 1000;
                //结束日期对象
                enddate = Date.UTC(parseInt(endtime.substr(0, 4), 10),
                    parseInt(endtime.substr(5, 2), 10) - 1,
                    parseInt(endtime.substr(8, 2), 10),
                    parseInt(endtime.substr(11, 2), 10) + offsethour,
                    parseInt(endtime.substr(14, 2), 10), 00) / 1000;
            }
            if (srtdate >= enddate) {
                view.setCallMsg('开始时间必须小于结束时间');
                return;
            }
            var resultDoc = view.G('page31_result');
            if ((enddate - srtdate) / (24 * 3600) > 31) {
                view.setCallMsg('最大允许查询30天内的挂靠，请重新指定时间区间！');
                resultDoc.innerHTML = "";
                return;
            }
            var calls = new shipxyAPI.PortOfCallByShipPort();
            view.setCallMsg('正在查询挂靠港历史，请稍候...');
            var c1 = view.G('page31_shipId').value;
            var c2 = view.G('page31_imo').value;
            var c3 = view.G('page31_name').value;
            var c4 = view.G('page31_callsign').value;
            var c5 = view.G('page31_portId').value;
            var c6 = srtdate;
            var c7 = enddate;
            var c8 = datetype;
            //获取挂靠港历史数据   shipId, imo, shipname, callsign, portid ,startTime, endTime, timetype
            calls.getCalls({ shipId: c1, imo: c2, shipname: c3, callsign: c4, portid: c5, startTime: c6, endTime: c7, timetype: c8 }, function (status) {
                if (status == 0) {//成功
                    view.setCallMsg('');
                    var data = this.data;
                    if (data && data.records && data.records.length > 0) {//有数据
                        data.records.reverse();
                        var c = '', len = 0, ndata = data.records;
                        c += '<table id="page29_table" class="dataTable" cellspacing="0"><caption style="">查询结果<span>(' + (ndata.length ? ndata.length : 0) + ')</span></caption>' +
                        '<thead><tr><th scope="col">港口中文</th><th scope="col">港口英文</th><th scope="col">港口ID</th><th scope="col">港口CODE</th><th scope="col">码头名称</th>' +
                        '<th scope="col">主港中文</th><th scope="col">主港英文</th><th scope="col">主港ID</th><th scope="col">主港CODE</th><th scope="col">国家中文</th>' +
                        '<th scope="col">国家英文</th><th scope="col">国家CODE</th><th scope="col">ATA</th><th scope="col">ATB</th><th scope="col">ATD</th>' +
                        '<th scope="col">到港吃水</th><th scope="col">离港吃水</th><th scope="col">港口作业时间</th><th scope="col">码头作业时间</th>' +
                        '</tr></thead><tbody>';
                        var d = null, i = 0;
                        for (len = ndata.length; i < len; i++) {
                            d = ndata[i];
                            c += '<tr class="' + (i % 2 == 1 ? 'odd' : '') + '">' +
                            '<td title="' + d.portname_cn + '">' + d.portname_cn + '</td><td title="' + d.portname_en + '">' + d.portname_en + '</td><td title="' + d.portid + '">' + d.portid + '</td><td title="' + d.portcode + '">' + d.portcode + '</td><td title="' + d.terminalname + '">' + d.terminalname + '</td>' +
                            '<td title="' + d.masterportname_cn + '">' + d.masterportname_cn + '</td><td title="' + d.masterportname_en + '">' + d.masterportname_en + '</td><td title="' + d.masterportid + '">' + d.masterportid + '</td><td title="' + d.masterportcode + '">' + d.masterportcode + '</td><td title="' + d.country_cn + '">' + d.country_cn + '</td>' +
                            '<td title="' + d.country_en + '">' + d.country_en + '</td><td title="' + d.countrycode + '">' + d.countrycode + '</td><td title="' + d.ata + '">' + d.ata + '</td><td title="' + d.atb + '">' + d.atb + '</td><td title="' + d.atd + '">' + d.atd + '</td>' +
                            '<td title="' + d.arrivaldraught + '">' + d.arrivaldraught + '</td><td title="' + d.departuredraught + '">' + d.departuredraught + '</td><td title="' + d.stayinport + '">' + d.stayinport + '</td><td title="' + d.stayinterminal + '">' + d.stayinterminal + '</td>' +
                            '</tr>';
                        }
        ...
        //获取船舶挂靠当前状态
        searchShipcallInfo: function () {
            var result = view.G('page32_result');
            var ships = new shipxyAPI.ShipStatus();
            var c1 = view.G('page32_shipId').value;
            var c2 = view.G('page32_imo').value;
            var c3 = view.G('page32_name').value;
            var c4 = view.G('page32_callsign').value;
            //请求船舶数据 shipId, imo, shipname, callsign
            ships.getCalls({ shipId: c1, imo: c2, shipname: c3, callsign: c4 }, function (status) {
                var data = this.data;
                if (status == 0) {//成功
                    var c = '';
                    if (data && data.records && data.records.length > 0) {//有数据
                        var d = data.records[0];
                        c += view.createTableByShipCallData(d); //根据数据生成数据表
                        action.locateShipById(d.mmsi.toString()); //定位到地图上显示
                    } else {//无数据
                        c += '没有该船舶数据';
                    }
                    result.innerHTML = c;
        ...
        //获取船舶档案
        searchShipRecord: function () {
            var result = view.G('page33_result');
            var ships = new shipxyAPI.ShipParticular();
            var c1 = view.G('page33_shipId').value;
            var c2 = view.G('page33_imo').value;
            var c3 = view.G('page33_name').value;
            var c4 = view.G('page33_callsign').value;
            //请求船舶数据 shipId, imo, shipname, callsign
            ships.getShipParticular({ shipId: c1, imo: c2, shipname: c3, callsign: c4 }, function (status) {
                var d = this.data;
                if (status == 0) {//成功
                    var c = '';
                    if (d && d.data && d.data.length > 0) {//有数据
                        var nd = d.data[0];
                        c += view.createTableByShipRecord(nd); //根据数据生成数据表
                        action.locateShipById(nd.MMSI); //定位到地图上显示
                    } else {//无数据
                        c += '没有该船舶数据';
                    }
                    result.innerHTML = c;
        ...
        addMarker: function () {
            ...
            //增加Marker后初始化Marker坐标显示37.2 122
            view.G('page20_lat').value = 37.2;
            view.G('page20_lng').value = 122;
        ...
        //新增加Marker经纬度坐标
        showMarkerLatLng: function (markLat, markLng) {
            view.G('page20_lat').value = markLat;
            view.G('page20_lng').value = markLng;
            var markerLatLng = [{ lat: markLat, lng: markLng }];
            action.showCodeFrameLatLng(markerLatLng);
        },
        //添加线
        addPolyline: function () {
            ...
            for (var a = 0; a < data.length; a++) {
                if (a == data.length - 1) {
                    defaultPoints += "[" + data[a].lat + "," + data[a].lng + "]";
                } else {
                    defaultPoints += "[" + data[a].lat + "," + data[a].lng + "],";
                }
            }
            view.G('page21_lineLatLng').value = defaultPoints;
        ...
        //新增加线顶点经纬度坐标
        showLineLatLng: function (lines) {

            var linePoints = "";
            for (var a = 0; a < lines.length; a++) {
                if (a == lines.length - 1) {
                    linePoints += "[" + lines[a].lat + "," + lines[a].lng + "]";
                } else {
                    linePoints += "[" + lines[a].lat + "," + lines[a].lng + "],";
                }

            }
            view.G('page21_lineLatLng').value = linePoints;
            action.showCodeFrameLatLng(lines);
        },
        //添加面
        addPolygon: function () {
            map.removeAllOverlay();
            ...
            view.G('page22_polygonLatLng').value = defaultPoints;
        ...
        //新增加面顶点经纬度坐标
        showPolygonLatLng: function (polygons) {

            var polygonPoints = "";
            for (var a = 0; a < polygons.length; a++) {
                if (a == polygons.length - 1) {
                    polygonPoints += "[" + polygons[a].lat + "," + polygons[a].lng + "]";
                } else {
                    polygonPoints += "[" + polygons[a].lat + "," + polygons[a].lng + "],";
                }

            }
            view.G('page22_polygonLatLng').value = polygonPoints;
            action.showCodeFrameLatLng(polygons);
        },
        //向code页面动态返回坐标
        showCodeFrameLatLng: function (dataArr) {
            var dataContent = "";
            for (var i = 0; i < dataArr.length; i++) {
                dataContent += "            data[" + i + "] = new shipxyMap.LatLng(" + dataArr[i].lat + ", " + dataArr[i].lng + ");\n";
            }
            window.frames['codeFrame'].document.body.innerHTML = window.frames['codeFrame'].document.body.innerHTML.replace(/DATASTART([\s\S]*)DATAEND/, 'DATASTART\n' + dataContent + '            //DATAEND');
        },
        //获取船队列表
        ...
        openWin: function () {
            if (!this.testWin) {
                this.testWin = new XWin({
                    modal: false,
                    width: 420,
                    position: [360, 200],
                    title: '窗口标题',
                    content: '<div style="text-align:center;padding:10px;"><a href="#">窗口内容</a></div>',
                    onhide: function () {
                        alert('窗口被关闭了！');
                    }
                });
        ...
        //关闭窗口
        closeWin: function () {
            if (this.testWin && !this.testWin.ishide()) this.testWin.hide();
        }
    };

    view = {
        listCheckedCode: 0,//当前选定
        G: function (id) {
        ...
        openWin: function (title, content) {
            ...
            win.setParam({ width: 420 });//初始化
            if (/29|30|31/.test(view.listCheckedCode)) win.setParam({ width: 650 });
            win.show(); //显示
            win.setPosition([document.body.clientWidth - 450, 120]);//初始化
            if (/29|30|31/.test(view.listCheckedCode)) win.setPosition([document.body.clientWidth - 680, 120]);
        },
        //关闭窗口
        closeWin: function () {
            if (win) {
        ...
        //设置设置靠港历史查询提示消息
        setCallMsg: function (msg) {
            var callMsg = this.G("callMsg");
            callMsg.innerHTML = msg;
            if (msg) {
                callMsg.style.display = 'inline-block';
            } else {//当是空消息，隐藏
        ...
        //根据船舶数据生成表格
        createTableByShipCallData: function (d) {

            var currentseaarea = ((d.currentseaarea && d.currentseaarea.length) > 0 ? d.currentseaarea[0] : "");
            var previousport = ((d.previousport && d.previousport.length) > 0 ? d.previousport[0] : "");
            var currentport = ((d.currentport && d.currentport.length) > 0 ? d.currentport[0] : "");

            return '<table class="xinfo"><tr>'
            + '<td class="l">船名:</td><td class="o" title="' + d.shipname + '">' + d.shipname + '</td>'
            + '<td class="l">呼号:</td><td class="o" title="' + d.callsign + '">' + d.callsign + '</td>'
            + '</tr><tr>'
            + '<td class="l">IMO:</td><td class="o" title="' + d.imo + '">' + d.imo + '</td>'
            + '<td class="l">MMSI:</td><td class="o" title="' + d.mmsi + '">' + d.mmsi + '</td>'
            + '</tr><tr>'
            + '<td class="l">船型中文:</td><td class="o" title="' + d.shiptype_cn + '">' + d.shiptype_cn + '</td>'
            + '<td class="l">船型英文:</td><td class="o" title="' + d.shiptype_en + '">' + d.shiptype_en + '</td>'
            + '</tr><tr>'
            + '<td class="l"> 当前海域:</td><td class="o" title="' + currentseaarea + '">' + currentseaarea + '</td>'
            + '<td class="l">经度:</td><td class="o" title="' + d.lon + '">' + d.lon + '</td>'
            + '</tr><tr>'
            + '<td class="l">纬度:</td><td class="o" title="' + d.lat + '">' + d.lat + '</td>'
            + '<td class="l">上一港英文:</td><td class="o" title="' + (previousport ? previousport.portname_en : "") + '">' + (previousport ? previousport.portname_en : "") + '</td>'
            + '</tr><tr>'
            + '<td class="l">上一港ID:</td><td class="o" title="' + (previousport ? previousport.portid : "") + '">' + (previousport ? previousport.portid : "") + '</td>'
            + '<td class="l">上一港CODE:</td><td class="o" title="' + (previousport ? previousport.portcode : "") + '">' + (previousport ? previousport.portcode : "") + '</td>'
            + '</tr><tr>'
            + '<td class="l">上一港ATA:</td><td class="o" title="' + (previousport ? previousport.ata : "") + '">' + (previousport ? previousport.ata : "") + '</td>'
            + '<td class="l">上一港ATB:</td><td class="o" title="' + (previousport ? previousport.atb : "") + '">' + (previousport ? previousport.atb : "") + '</td>'
            + '</tr><tr>'
            + '<td class="l">上一港ATD:</td><td class="o" title="' + (previousport ? previousport.atd : "") + '">' + (previousport ? previousport.atd : "") + '</td>'
            + '<td class="l">当前港英文:</td><td class="o" title="' + (currentport ? currentport.portname_en : "") + '">' + (currentport ? currentport.portname_en : "") + '</td>'
            + '</tr><tr>'
            + '<td class="l">当前港ID:</td><td class="o" title="' + (currentport ? currentport.portid : "") + '">' + (currentport ? currentport.portid : "") + '</td>'
            + '<td class="l">当前港CODE:</td><td class="o" title="' + (currentport ? currentport.portcode : "") + '">' + (currentport ? currentport.portcode : "") + '</td>'
            + '</tr><tr>'
            + '<td class="l">当前港ATA:</td><td class="o" title="' + (currentport ? currentport.ata : "") + '">' + (currentport ? currentport.ata : "") + '</td>'
            + '<td class="l">当前港ATB:</td><td class="o" title="' + (currentport ? currentport.atb : "") + '">' + (currentport ? currentport.atb : "") + '</td>'
            + '</tr></table>';
        },
        createTableByShipRecord: function (d) {

            return '<table class="dataTable"><tr>'
            + '<td class="l">船名:</td><td class="o" title="' + d.ShipName + '">' + d.ShipName + '</td>'
            + '<td class="l">呼号:</td><td class="o" title="' + d.CallSign + '">' + d.CallSign + '</td>'
            + '</tr><tr>'
            + '<td class="l">IMO:</td><td class="o" title="' + d.IMO + '">' + d.IMO + '</td>'
            + '<td class="l">MMSI:</td><td class="o" title="' + d.MMSI + '">' + d.MMSI + '</td>'
            + '</tr><tr>'
            + '<td class="l">船长:</td><td class="o" title="' + d.Length + '">' + d.Length + '</td>'
            + '<td class="l">船宽:</td><td class="o" title="' + d.MouldWidth + '">' + d.MouldWidth + '</td>'
            + '</tr><tr>'
            + '<td class="l">国籍:</td><td class="o" title="' + d.FlagCountry + '">' + d.FlagCountry + '</td>'
            + '<td class="l">建造时间:</td><td class="o" title="' + d.BuildDate + '">' + d.BuildDate + '</td>'
            + '</tr><tr>'
            + '<td class="l">船级社:</td><td class="o" title="' + d.ClassName + '">' + d.ClassName + '</td>'
            + '<td class="l">船舶类型1:</td><td class="o" title="' + d.ShiptypeLevel5SubGroup + '">' + d.ShiptypeLevel5SubGroup + '</td>'
            + '</tr><tr>'
            + '<td class="l">船舶类型2:</td><td class="o" title="' + d.ShiptypeGroup + '">' + d.ShiptypeGroup + '</td>'
            + '<td class="l">船舶状态:</td><td class="o" title="' + d.ShipStatus + '">' + d.ShipStatus + '</td>'
            + '</tr><tr>'
            + '<td class="l">总吨:</td><td class="o" title="' + d.GrossTonnage + '">' + d.GrossTonnage + '</td>'
            + '<td class="l">净吨:</td><td class="o" title="' + d.NetTonnage + '">' + d.NetTonnage + '</td>'
            + '</tr><tr>'
            + '<td class="l">载重吨:</td><td class="o" title="' + d.DeadWeight + '">' + d.DeadWeight + '</td>'
            + '<td class="l">TEU:</td><td class="o" title="' + d.TEU + '">' + d.TEU + '</td>'
            + '</tr><tr>'
            + '<td class="l">最大速度:</td><td class="o" title="' + d.SpeedMax + '">' + d.SpeedMax + '</td>'
            + '<td class="l">经济航速:</td><td class="o" title="' + d.SpeedService + '">' + d.SpeedService + '</td>'
            + '</tr><tr>'
            + '<td class="l">夏季吃水:</td><td class="o" title="' + d.Draught + '">' + d.Draught + '</td>'
            + '<td class="l">船舶所属公司:</td><td class="o" title="' + d.GroupCompany + '">' + d.GroupCompany + '</td>'
            + '</tr><tr>'
            + '<td class="l">船舶管理公司:</td><td class="o" title="' + d.ShipManagerCompany + '">' + d.ShipManagerCompany + '</td>'
            + '<td class="l">船舶经营公司:</td><td class="o" title="' + d.OperatorCompany + '">' + d.OperatorCompany + '</td>'
            + '</tr><tr>'
            + '<td class="l">建造地:</td><td class="o" title="' + d.BuildCountry + '">' + d.BuildCountry + '</td>'
            + '<td class="l">P&I:</td><td class="o" title="' + d.PandIClub + '">' + d.PandIClub + '</td>'
            + '</tr><tr>'
            + '<td class="l">更新时间:</td><td class="o" title="' + d.DataUpdateTime + '">' + d.DataUpdateTime + '</td>'
            + '<td class="l"></td><td class="o" title=""></td>'
